...
async def create_admin():
    if repository.get_user("admin1") is None:
        user1 = {
            ...
            "password": "password1",
            "name": "name1",
            "role": repository.Role.DRUNKMASTER.value,
        }
...
async def create_ingredients():
    if repository.get_ingredient("Белый ром") is None:
        await ingredient_crud.post_ingredient(
            {
                "name": "Белый ром",
...
async def create_cocktails():
    ...
    if "Мохито" not in cocktail_names:
        await cocktail_crud.post_cocktail(
            {
                "name": "Мохито",
...
Укрась веточкой мяты и долькой лайма""",
    ...
    if "Дайкири" not in cocktail_names:
        await cocktail_crud.post_cocktail(
            {
                "name": "Дайкири",
...
Перелей через стрейнер в охлажденное шампанское блюдце""",
    ...
    if "Клубничный дайкири" not in cocktail_names:
        await cocktail_crud.post_cocktail(
            {
...
Перелей через стрейнер в шампанское блюдце""",
    ...
    if "Горячий апельсин" not in cocktail_names:
        await cocktail_crud.post_cocktail(
            {
...
Укрась нарезанной клубникой""",
    ...
    if "Джин тоник" not in cocktail_names:
        await cocktail_crud.post_cocktail(
            {
                "name": "Джин тоник",
        ...
        await user_crud.create_user(
            {
                ...
                "name": "test_user_1",
                "role": repository.Role.USER.value,
            }
        )
...
async def create_comments():
    ...
    for cocktail in list(repository.get_cocktails()):
        await comment_crud.post_comment(
            str(cocktail["_id"]), str(user1["_id"]), "test1", 1
        )
        await comment_crud.post_comment(
            str(cocktail["_id"]), str(user2["_id"]), "test2", 2
        )
        await comment_crud.post_comment(
            str(cocktail["_id"]), str(user3["_id"]), "test3", 3
        )
