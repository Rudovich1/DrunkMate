...
async def post_comment(cocktail_id: str, author_id: str, text: str, rating: int):
    ...
    for comment in comments:
        ...
        if str(author) == author_id:
            new_rating = (
                cocktail["rating"] * len(cocktail["comments"])
                - comment["rating"]
                + rating
            ) / len(cocktail["comments"])
    ...
    if cocktail["rating"] is None:
        ...
        new_rating = (cocktail["rating"] * len(cocktail["comments"]) + rating) / (
            len(cocktail["comments"]) + 1
        )

    cocktail["comments"].append(ObjectId(comment_id))
    repository.put_cocktail(
        cocktail_id, {"rating": new_rating, "comments": cocktail["comments"]}
    )
...
async def delete_comment(comment_id: str, cocktail_id: str):
    ...
    if len(cocktail["comments"]) == 1:
        ...
        cocktail["rating"] = (
            cocktail["rating"] * len(cocktail["comments"]) - comment["rating"]
        ) / (len(cocktail["comments"]) - 1)
        cocktail["comments"].remove(ObjectId(comment_id))

    repository.put_cocktail(
        cocktail_id, {"rating": cocktail["rating"], "comments": cocktail["comments"]}
    )
